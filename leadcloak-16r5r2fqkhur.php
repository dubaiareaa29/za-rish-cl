...
$campaignId = '16r5r2fqkhur';

$campaignSignature = 'vHbL9S0NcDyRRBZzyVf1Dk67NVygs6u9pIEvNgRLYE03rhNWUq';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16r5r2fqkhur', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16r5r2fqkhur'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16r5r2fqkhur');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: vHbL9S0NcDyRRBZzyVf1Dk67NVygs6u9pIEvNgRLYE03rhNWUq';
...
{
    return array_key_exists('16r5r2fqkhur', $_COOKIE) ? $_COOKIE['16r5r2fqkhur'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16r5r2fqkhur', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16r5r2fqkhur.log';
